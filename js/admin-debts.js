...
export function createAdminDebtsController(ctx) {
  ...
  function render() {
    ...
    accounts.forEach(account => {
      ...
      row.innerHTML = `<td><strong class="admin-table__primary">${escapeHtml(getProfileName(account.member))}</strong><span class="admin-table__secondary">${escapeHtml(account.member?.phone || account.member?.email || '—')}</span></td><td><strong class="${balance > 0 ? 'finance-amount finance-amount--expense' : 'finance-amount'}">${escapeHtml(money(balance))}</strong></td><td>${formatDateTime(account.updated_at)}</td><td><span class="${balance > 0 ? 'ui-badge ui-badge--danger' : 'ui-badge ui-badge--success'}">${balance > 0 ? 'Açıq borc' : 'Ödənilib'}</span></td><td>${balance > 0 ? `<button type="button" class="ui-button ui-button--primary" data-debt-pay="${escapeHtml(account.member_id)}"><span class="ui-button__label">Ödəniş</span></button>` : '—'}</td>`;
      tbody.append(row);
    });
    root.append(table);
