...
// account.id YOXDUR.
  ...
  if (!append) { resetRemotePage('debtTransactions'); resetListLimit('debtTransactions'); }
...
// created_at əməliyyat timestamp-ıdır.
  ...
  const append = options.append === true;
  if (!append) { resetRemotePage('finance'); resetListLimit('finance'); }
  return loadRemotePage('finance', { append });
}

) {
  const append = options.append === true;
  if (!append) { resetRemotePage('stockMovements'); resetListLimit('stockMovements'); }
  ...
  const append = options.append === true;
  try {
    if (!append) { resetRemotePage('cash'); resetListLimit('cash'); }
    const [, balanceResult] = await Promise.all([loadRemotePage('cash', { append }), supabase.rpc(RPC.getCashRegisterBalance)]);
    ...
    if (!append) state.cashRegisterEntries = [];
    return state.cashRegisterEntries;
  } finally { state.loading.cash = false; }
}
