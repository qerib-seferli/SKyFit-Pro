...
export function createAdminFinanceController(ctx) {
  ...
  function renderCash() {
    ...
    visibleListItems('cash', state.cashRegisterEntries).forEach(entry => {
      ...
      row.innerHTML = `<td><strong class="admin-table__primary">${formatDate(entry.entry_date)}</strong><span class="admin-table__secondary">${formatTime(entry.created_at)}</span></td><td><span class="${incoming ? 'ui-badge ui-badge--success' : 'ui-badge ui-badge--danger'}">${incoming ? 'Kassaya daxil' : 'Kassadan çıxış'}</span></td><td>${escapeHtml(entry.category || '—')}</td><td>${escapeHtml(entry.description || '—')}</td><td><strong class="${incoming ? 'finance-amount finance-amount--income' : 'finance-amount finance-amount--expense'}">${incoming ? '+' : '−'} ${escapeHtml(money(entry.amount))}</strong></td>`;
      tbody.append(row);
    });
    root.append(table);
  ...
  function render() {
    ...
    visibleListItems('finance', entries).forEach(entry => {
      ...
      row.innerHTML = `<td><strong class="admin-table__primary">${formatDate(entry.entry_date)}</strong><span class="admin-table__secondary">${entry.created_at ? formatTime(entry.created_at) : ''}</span></td><td><span class="${type === 'income' ? 'ui-badge ui-badge--success' : 'ui-badge ui-badge--danger'}">${type === 'income' ? 'Mədaxil' : 'Məxaric'}</span></td><td>${escapeHtml(entry.category || '—')}</td><td>${escapeHtml(entry.description || '—')}</td><td><span class="ui-badge ui-badge--neutral">${escapeHtml(paymentMethodLabel(entry.payment_method))}</span></td><td><span class="ui-badge ui-badge--neutral">${escapeHtml(financeReferenceLabel(entry.reference_type))}</span></td><td><strong class="${type === 'income' ? 'finance-amount finance-amount--income' : 'finance-amount finance-amount--expense'}">${type === 'income' ? '+' : '−'} ${escapeHtml(money(ledgerAmount(entry)))}</strong></td>`;
      tbody.append(row);
    });
    root.append(table);
